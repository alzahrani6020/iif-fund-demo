...
| اختصارات | `/legal`، `/privacy`، `/disclaimer`، `/contact` → محلي/خادم |
...
3. **Git:** `git add` / `commit` / `push` إلى `main`.
4. **بعد أول نشر:** افتح النطاق `https://<user>.github.io/<repo>/` وجرّب `/`، `/fund`، `/gov`، `/privacy`، `/legal`.
5. **مشاركة اجتماعية:** إن احتجت رابط `og:image` مطلقاً 100%، عيّن النطاق النهائي في إعدادات GitHub Pages أو حدّث الوسوم يدوياً (بعض المنصات تقبل المسارات النسبية مع النطاق تلقائياً).
