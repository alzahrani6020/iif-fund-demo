...
export function Formation({ lang }: { lang: Lang }) {
  ...
  return (
    <section id="company-formation" className="py-16 sm:py-24 bg-surface-50">
      <div className="container-modern">
        ...
        <div className="mt-12 grid sm:grid-cols-2 gap-5">
          {t.formation.cards.map((card, i) => (
            ...
            >
              <h3 className="text-lg font-bold text-saudi-700">{card.title}</h3>
              <p className="mt-2 text-surface-500 text-sm leading-relaxed">{card.desc}</p>
            </motion.div>
          ))}
        </div>

        <motion.div initial={{ opacity: 0, y: 15 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mt-5 card-modern p-6 bg-surface-50">
