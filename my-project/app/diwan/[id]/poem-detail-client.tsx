...
import { Heart, Share2, Play, BookOpen, Calendar, ChevronLeft, ChevronRight } from "lucide-react"
import Link from "next/link"
import { updatePoem, getPoems, type Poem } from "@/lib/data-store"
    ...
    <main className="min-h-screen">
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 pb-8">
        <div className="flex items-center justify-between gap-4">
          {prevPoem ? (
            <Link
              href={`/diwan/${prevPoem.id}`}
              className="flex items-center gap-2 text-muted-foreground hover:text-accent transition-colors"
            >
              <ChevronRight className="h-5 w-5" />
              <div className="text-right">
                <p className="text-xs text-muted-foreground">السابقة</p>
                <p className="text-sm font-medium">{prevPoem.title}</p>
              </div>
            </Link>
          ) : (
            <div />
          ...
          {nextPoem ? (
            <Link
              href={`/diwan/${nextPoem.id}`}
              className="flex items-center gap-2 text-muted-foreground hover:text-accent transition-colors"
            >
              <div className="text-left">
                <p className="text-xs text-muted-foreground">التالية</p>
                <p className="text-sm font-medium">{nextPoem.title}</p>
              </div>
